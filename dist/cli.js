...
function formatZodError(error) {
  return error.issues.map((issue) => {
    const path6 = issue.path.length > 0 ? issue.path.join(".") : "config";
    return `${path6}: ${issue.message}`;
  }).join("; ");
}
async function loadConfig(path6) {
  let raw;
  try {
    raw = await readFile(path6, "utf8");
  } catch {
    throw new Error(`Unable to read config file at ${path6}`);
  }
  let parsed;
  try {
    parsed = JSON.parse(raw);
  } catch {
    throw new Error(`Invalid JSON in config file at ${path6}`);
  }
...
async function writeJson(path6, data) {
  await ensureDir(dirname(path6));
  const content = JSON.stringify(data, null, 2);
  await writeFile(path6, content, "utf8");
}
async function writeText(path6, content) {
  await ensureDir(dirname(path6));
  await writeFile(path6, content, "utf8");
}
